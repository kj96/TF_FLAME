...
def build_texture_from_image(source_img_fname, target_mesh_fname, target_scale_fname, texture_mapping, out_path):
    """
        source_img_fname    : INPUT face image
        target_mesh_fname   : genrated face mesh
        target_scale_fname  : genrated image projection scale         ====            ???????
        texture_mapping     : FLAME texture mapping
        out_path            : 
    """
    ...
    # Load taget IMAGE and genrataed MESH
    ...
    # load projection scale
    target_scale = np.load(target_scale_fname)
    # laod texture mapping
    ...
    #       target_mesh    :   genrated face mesh
    #       target_scale   :   genrated image projection scale         ====            ???????
    #       texture_data   :   FLAME texture mapping
    #   OUTPUT:
    #       texture_map
    texture_map = compute_texture_map(source_img, target_mesh, target_scale, texture_data)

    # Output texture and object
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source_img', default='./data/jeff1.png', help='source image filename')
    parser.add_argument('--target_mesh', default='./results/jeff1.obj', help='target mesh filename')
    parser.add_argument('--target_scale', default='./results/jeff1_scale.npy', help='scale of the target mesh for the image projection')
